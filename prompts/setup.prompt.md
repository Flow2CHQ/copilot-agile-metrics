...
description: One-time setup wizard. Asks about your project's structure once, then generates personalised, ready-to-use versions of all analysis prompts in prompts/configured/ — no re-explaining needed afterwards.
argument-hint: "Run with no arguments — the wizard will ask everything"
mode: agent
---
...
3. Use the answers to generate five customised `.prompt.md` files in `prompts/configured/`
...
Create the directory `prompts/configured/` if it does not exist.

For each of the five prompts below, create a new file in `prompts/configured/` using the exact content specified. Replace every `{placeholder}` with the confirmed configuration values.
...
### File 1 — `prompts/configured/sprint-analysis.prompt.md`
...
> **Configuration** — generated by `setup.prompt.md`
...
### File 2 — `prompts/configured/retro-input.prompt.md`
...
### File 3 — `prompts/configured/monte-carlo-forecast.prompt.md`
...
### File 4 — `prompts/configured/burndown-chart.prompt.md`
...
> 2. How many issues were **in scope at sprint start**? *(The total number of issues that were open and assigned to {current_sprint} on day 1 — estimate is fine)*
...
Fetch all issues for sprint **{current_sprint}** in `{owner/repo}` that were closed between the sprint start date and today.
Filter: {resolved_filter_strategy}

Record `closedAt` date for each. Also count currently open issues.

Sprint end date = sprint start date + {sprint_duration_days} days.
...
### File 5 — `prompts/configured/stakeholder-update.prompt.md`
...
✅ **Setup complete!** Your configured prompts are ready in `prompts/configured/`.

| Prompt | File |
|--------|------|
| Sprint Analysis | `prompts/configured/sprint-analysis.prompt.md` |
| Retro Input | `prompts/configured/retro-input.prompt.md` |
| Monte Carlo Forecast | `prompts/configured/monte-carlo-forecast.prompt.md` |
| Burndown Chart | `prompts/configured/burndown-chart.prompt.md` |
| Stakeholder Update | `prompts/configured/stakeholder-update.prompt.md` |
